# Jochen: This is mostly the PyTorch's MNIST dataset.
...
class MnistTrain(Mnist):
    def __init__(self, args, root, num_workers=2, num_examples=-1):
...
class MnistValid(Mnist):
    def __init__(self, args, root, num_workers=3, num_examples=-1):
        d = os.path.dirname(root)
        if not os.path.exists(d):
            os.makedirs(d)
        # transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
...
factory.register("MnistTrain", MnistTrain)
factory.register("MnistValid", MnistValid)
